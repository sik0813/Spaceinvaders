...
public class Board extends JPanel implements Runnable, Commons {
	...
	public void gameInit() {
		...
		for (int i = 0; i < 4; i++) {
			for (int j = 0; j < 6; j++) {
				Alien alien = new Alien(alienX + (18 * 3) * j, alienY + (18* 2) * i);// 18
	...
	public void paint(Graphics g) {
		...
		//g.fillRect(0, d.height/2,d.width, d.height);
		g.setColor(Color.green);
	...
	public void gameOver() {
		...
		g.drawString(message, (BOARD_WIDTH - metr.stringWidth(message)) / 2,
				BOARD_WIDTH / 2);
		g.drawRect(50, BOARD_WIDTH - 66, BOARD_WIDTH - 100, 20);
		g.drawString(
				"Press Any key to continue",
				(BOARD_WIDTH - metr.stringWidth("Press Any key to continue")) / 2,
				BOARD_WIDTH - 50);
	}
	...
	private class TAdapter extends KeyAdapter {
		...
		public void keyPressed(KeyEvent e) {
			...
			if (ingame != true) {
				if (e.getKeyCode() < KeyEvent.KEY_LOCATION_UNKNOWN ||e.getKeyCode() > KeyEvent.KEY_LOCATION_NUMPAD &&e.getKeyCode() != KeyEvent.VK_X) {
					new SpaceInvaders();
					}
			}
		}
